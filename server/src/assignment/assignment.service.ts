...
export class AssignmentService {
  ...
  async create(createAssignmentDto: CreateAssignmentDto): Promise<Assignment> {
    ...
    if (createAssignmentDto.teacherFile) {
      const uploadResult = await uploadSingleFileToCloudinary(createAssignmentDto.teacherFile, 'assignments');
  ...
  async update(assignmentId: string, updateAssignmentDto: UpdateAssignmentDto): Promise<Assignment> {
    ...
    if (updateAssignmentDto.teacherFile) {
      if (existingAssignment.teacherFilePath) {
        const publicId = extractPublicIdFromUrl(existingAssignment.teacherFilePath);
        await deleteFileFromCloudinary(publicId);
      }
      const uploadResult = await uploadSingleFileToCloudinary(updateAssignmentDto.teacherFile, 'assignments');
